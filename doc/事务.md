...
###### mysql查看默认的系统级: select @@global.tx_isolation;mysql默认的事务隔离级别为repeatable-read：select @@tx_isolation;设置当前事务隔离级别：set session transaction isolation level repeatable read; start transaction;
## 事务传播机制
