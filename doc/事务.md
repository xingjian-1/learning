****
例子：某人要在商店使用电子货币购买100元的东西，当中至少包括两个操作 <br/>

    该人账户减少100元
    商店账户增加100元

支持事务的数据库管理系统（transactional DBMS）就是要确保以上两个操作都能完成，或一起取消；否则就会出现100元平白消失或出现的情况
## 事务的特性ACID
#### `原子性(Atomicity)`
###### 一个事务包含的所有的操作都是一个不可分割的整体，要么全部成功，要么全部失败。

           转账：账户A给账户B转账1W元。那数据库假设需要 账户A扣1W，账户B加1W，记录一条流水。如果流水没记录成功，那整体回滚，账户A还是原来的金额，账户B的金额也没变。这就是原子性的体现。
#### `一致性(Consistency)`
###### 一个事务执行之前和执行之后操作的数据都必须处于一致性状态。

           例如账户A和账户B两个账户的钱加起来一共是1W，不管账户A和账户B之间如何转账、转几次账，事务结束后两个账户的钱相加起来应该还是1W，这就是事务的一致性。数据的一致性(C)是最终目标，其它的特性都是为了达成这个目标。
#### `持久性(Durability)`
###### 持久性是指一个事务一但被提交了，那么对数据库数据的改变就是永久的。
#### `隔离性(Isolation)`
###### 事务跟事务之间对于数据的操作是完全隔离的，一个事务执行期间不会受到其他事务的干扰。关于事务的隔离性，数据库提供了多种隔离级别。
## 事务隔离级别  
* 未提交的读(READ UNCOMMITTED)：是最低的隔离级别，会出现脏读、不可重复读、幻读，事务可以看到其他事务“尚未提交”的修改。
* 提交的读（Read Committed）：会出现不可重复读和幻读，但不会出现脏读。可以通过“共享读锁”和“排他写锁”实现。
* 重复读（Repeatable Read）：会出现幻读，可以通过“共享读锁”和“排他写锁”实现。
* 序列化(Serializable)：最高的隔离级别，它要求事务序列化执行，事务只能一个接着一个地执行，不能并发执行。隔离级别越高，越能保证数据的完整性和一致性，但是对并发性能的影响也越大
...
###### `不可重复读`：在同一个事务里，对同一条数据两次读取，两次读取的数据不一样。事务 A 多次读取同一数据，事务 B 在事务A多次读取的过程中，对数据作了更新并提交，导致事务A多次读取同一数据时，结果 不一致
...
## 数据库事务总结
###### 务的（ACID）特性是由关系型数据库管理系统（RDBMS，数据库系统）来实现的。数据库管理系统采用日志来保证事务的原子性、一致性和持久性。日志记录了事务对数据库所做的更新，如果某个事务在执行过程中发生错误，就可以根据日志，撤销事务对数据库已做的更新，使数据库退回到执行事务前的初始状态。
## 事务传播机制
...
## 事务嵌套
###### 注意嵌套事务：propagation_nested 嵌套事务一个非常重要的概念就是内层事务依赖于外层事务。外层事务失败时，会回滚内层事务所做的动作。而内层事务操作失败并不会引起外层事务的回滚
## Spring事务
